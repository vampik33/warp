...
use crate::{
    crossterm_event_to_warp_event, TuiBuffer, TuiConstraint, TuiEventContext, TuiFrameRenderer,
    ...
    R: TuiTerminal,
{
    ...
    fn draw_if_dirty(&mut self, app: &App) -> io::Result<()> {
        ...
        let (buffer, cursor) = present(app, &self.root_view, size);
        let mut writer = self.terminal.writer();
        self.renderer.draw(&mut writer, &buffer, cursor)?;
...
/// Renders `root_view`'s element tree and paints it into a fresh buffer sized to
/// `size`, returning the buffer and the cursor position the view requests.
///
/// This is the self-contained presentation path the runtime uses; at the
/// milestone boundary it can be swapped for the dedicated presenter.
fn present<T>(
    app: &App,
    root_view: &TuiViewHandle<T>,
    size: TuiSize,
) -> (TuiBuffer, Option<(u16, u16)>)
where
    T: TuiView<RenderOutput = TuiRenderOutput>,
{
    let mut element = render_root(app, root_view);
    element.layout(TuiConstraint::tight(size));
    let area = TuiRect::from_size(size);
    let mut buffer = TuiBuffer::new(size);
    element.render(area, &mut buffer);
    let cursor = element.cursor_position(area);
    (buffer, cursor)
}
