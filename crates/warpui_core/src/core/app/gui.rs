...
//! single `#[cfg(not(feature = "tui"))] mod gui;` guard in `app.rs` now gates
//! the entire GUI fork; nothing here changes behavior or public API.
...
/// The GUI half of the window/event/draw loop. These methods were relocated
/// from the shared `app.rs` (where they were individually gated behind
/// `#[cfg(not(feature = "tui"))]`); the single module guard on `core/gui.rs`
/// now gates them all. The TUI build drives its own loop from `warpui_tui`'s
/// `TuiRuntime`.
impl AppContext {
    ...
    ///
    /// The TUI build uses a separate, neutral version below that performs only
    /// the backend-agnostic window-map bookkeeping + root-view creation; the TUI
    /// runtime (`warpui_tui`) drives its own event/draw loop.
