use ::local_control::protocol::{
    DriveGetParams, DriveGetResult, DriveListParams, DriveListResult, DriveObjectSummary,
...
pub(crate) fn drive_get(
    ...
    validate_drive_target(target, ActionKind::DriveGet)?;
    let params = action.params_as::<DriveGetParams>()?;
...
fn drive_object_get_result(object: &dyn CloudObject) -> Result<DriveGetResult, ControlError> {
...
fn drive_type_mismatch_error() -> ControlError {
    ControlError::new(
        ...
        "drive.get Drive object type does not match the resolved object",
    )
}
...
fn drive_unsupported_type_error() -> ControlError {
    ControlError::new(
        ErrorCode::UnsupportedAction,
        "drive.get content reads are not supported for this Drive object type",
    )
}
