...
2. Render the header row: chevron icon + `"<N> queued"` text (single label format from Figma node 6736:27438), wrapped in `Hoverable` with an `on_click` that dispatches `ToggleCollapsed`.
...
## Follow-ups
- Optional: persist queue across app restarts (called out as non-goal in `PRODUCT.md`).
- Optional: cross-conversation visibility (e.g. a small "queued elsewhere" indicator on tabs whose queue is non-empty).
- Optional: surface the queue in the conversation transcript / Cloud Mode details panel, since it's part of the conversation's state.
